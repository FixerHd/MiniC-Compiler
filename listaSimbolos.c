...
void imprimirLS(Lista simbolos) {
  PosicionLista p = inicioLS(simbolos);
  while (p != finalLS(simbolos)) {
    Simbolo s = recuperaLS(simbolos,p);
    if (s.tipo != CADENA ) {
      printf("_%s:\n",s.nombre);
      printf("          .word 0");
    }
    if (s.tipo == CADENA ) {
      printf("$str%d:\n", s.valor);
      printf("          .asciiz %s", s.nombre);
    }
    printf("\n");
    p = siguienteLS(simbolos,p);
  }
}
